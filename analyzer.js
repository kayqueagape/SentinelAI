...
async function loadToxicityModel() {
  ...
  try {
    ...
    console.warn('⚠️  TensorFlow unavailable — rule-based fallback active');
  }
  tfLoading = false;
}
loadToxicityModel().catch(() => {});

// ─── Rule-based toxicity (multilingual) ───────────────────────────────────────
const TOXIC_RULES = {
  TOXICITY:        [/\b(hate|kill|die|stupid|idiot|moron|dumb|loser|idiota|imbecil|burro|bobo|otário|chato|maldito|odeio|matar|destruir|inútil|inutil)\b/gi],
  SEVERE_TOXICITY: [/\b(fuck|shit|bastard|bitch|asshole|cunt|merda|porra|caralho|viado|fdp|imbecil)\b/gi],
  INSULT:          [/\b(ugly|fat|pathetic|worthless|useless|trash|garbage|lixo|incompetente|ridículo|ridiculo|estúpido|estupido|imbecil|palhaço)\b/gi],
  THREAT:          [/\b(threaten|hurt|harm|destroy|ruin|attack|beat|ameaç|agredir|machucar|acabar com)\b/gi, /i('ll| will) (kill|hurt|destroy|harm)/gi],
    ...
    (async () => {
      if (toxicityModel) {
        try {
          ...
          let maxScore = 0;
          for (const p of preds) { const prob = p.results[0]?.probabilities[1] ?? 0; if (prob > maxScore) maxScore = prob; }
